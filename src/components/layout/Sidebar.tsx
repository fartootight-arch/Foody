...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-64 bg-white border-r border-gray-200 flex-col shrink-0 h-full">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {nav.map(({ href, label, icon: Icon }) => (
            ...
            >
              <Icon className={cn("w-5 h-5", isActive(href) ? "text-orange-600" : "text-gray-400")} />
              {label}
            </Link>
          ))}
        </nav>
      ...
      {/* ── Mobile bottom nav ── */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex items-center justify-around z-50 h-14 px-1">
        {nav.slice(0, 6).map(({ href, label, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className={cn(
              "flex flex-col items-center gap-0.5 px-1 py-1 rounded-lg flex-1 transition-colors",
              isActive(href) ? "text-orange-600" : "text-gray-400"
            )}
          >
