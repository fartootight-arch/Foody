...
import { DIETARY_OPTIONS, ALLERGY_OPTIONS, RELATIONSHIP_OPTIONS, parseJsonArray } from "@/lib/utils";
  ...
  useEffect(() => {
    fetch(`/api/people/${id}`)
      ...
      .then((person) => {
        ...
        setDietary(parseJsonArray(person.dietary));
        setAllergies(parseJsonArray(person.allergies));
        setLikes(parseJsonArray(person.likes));
        setDislikes(parseJsonArray(person.dislikes));
    ...
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Update person details</p>
        </div>
        <ConfirmDialog
          trigger={
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              {likes.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {likes.map((l) => (
                    <span key={l} className="flex items-center gap-1 px-2.5 py-0.5 bg-green-50 text-green-700 rounded-full text-xs">
                      {l}
            ...
            <div className="space-y-2">
              ...
              {dislikes.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {dislikes.map((d) => (
                    <span key={d} className="flex items-center gap-1 px-2.5 py-0.5 bg-red-50 text-red-700 rounded-full text-xs">
                      {d}
